...
class Program
{
    ...
    private static void PlayGame(Game game)
    {
        while (!game.CheckIfFinished())
        {
            ...
            Console.WriteLine("\nskriv hvilket tall du vil åpne");
            var firstInput = Console.ReadLine();

            if (CheckForEmpty(firstInput)) break;

            var firstCardIndex = int.Parse(firstInput);
            var firstCard = game.OpenCard(firstCardIndex);

            Console.WriteLine("\nhvilket neste tall tror du er lik? ");
            var secondInput = Console.ReadLine();

            if (CheckForEmpty(secondInput)) break;

            var secondCardIndex = int.Parse(secondInput);
            var secondCard = game.OpenCard(secondCardIndex);
    ...
    private static bool CheckForEmpty(string? userInput)
    {
